...
export default function CreateTestForm() {
	...
	async function onSubmit(event) {
	  ...
	  try {
		...
		const response = await testService.create(courseId, episodeId, test);
  
		setSelectedEpisode((otherProperties) => ({
		  ...otherProperties,
		  test: response,
		}));
		...
		setModals((otherModals) => ({
		  ...otherModals,
		  createTest: { ...otherModals.createTest, show: false },
		}));
	  } catch (error) {
	...
	const questionsElements = questions.map((question) => (
	  <Question key={uniqid()} question={question} />
	));
